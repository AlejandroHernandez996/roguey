	...
	while (!ItemSpawnQueue.IsEmpty())
	{
		...
		if (ItemCache->ItemActor)
		{
			...
			else if (UWorld* World = GetWorld())
			{
				...
				TWeakObjectPtr<ArogueyItemActor> SpawnedActor = World->SpawnActorDeferred<ArogueyItemActor>(ItemCache->ItemActor, FTransform(SpawnRotation, SpawnLocation), nullptr, nullptr, ESpawnActorCollisionHandlingMethod::AlwaysSpawn);
...
void USpawnManager::ProcessNpcSpawners(int32 TickIndex)
{
	for (ArogueyNpcSpawner* NpcSpawner : NpcSpawners)
	{
		if (!NpcSpawner) continue;
		if (NpcSpawner->ShouldSpawn(TickIndex))
		{
			NpcSpawner->SpawnNpc(GridManager);
		}
	}
}
